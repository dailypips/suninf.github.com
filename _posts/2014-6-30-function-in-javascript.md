...
2. 以表达式方式定义的函数，仅仅是变量的声明提前了，但是变量的赋值没有提前，所以在函数表达式定义之前无法调用。
...
* 如果嵌套函数想访问外部函数的this值，可以在外部函数中将this的值存到一个变量中，如：
...
当实参数量比较多时，最好通过名/值对的形式包装成对象来做为一个参数传递
{% highlight javascript %}
